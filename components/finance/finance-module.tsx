...
export function FinanceModule({ onFeedback, billing }: { onFeedback: (message: string) => void; billing: React.ReactNode }) {
  ...
  async function saveExpense(event: React.FormEvent) {
    ...
    const data = { kind: tab, description: description.trim(), supplier: supplier.trim() || null, amount: Number(amount.replace(",", ".")), dueDate, status: "pending" as const };
    if (!db) writeLocal("financialExpenses", [{ id: `local-expense-${Date.now()}`, ...data }, ...expenses]);
    else await addDoc(collection(db, "academies", access.academyId, "financialExpenses"), { ...data, createdBy: access.userId, createdAt: serverTimestamp() });
    setDescription(""); setSupplier(""); setAmount(""); onFeedback("Conta registrada no financeiro.");
  }
