...
function WorkoutExecutionPreviewModal({ exercises, selectedExercises, exerciseDetails, onClose }: { exercises: ExerciseRecord[]; selectedExercises: string[]; exerciseDetails: Record<string, Omit<WorkoutExerciseDetail, "exerciseId" | "name">>; onClose: () => void }) {
  ...
  return <div className="training-preview-backdrop" role="dialog" aria-modal="true" aria-labelledby="workout-preview-title"><section className="training-preview-panel workout-execution-preview-panel"><header><div><span>PRÉVIA DO ALUNO · TREINO EM EXECUÇÃO</span><h2 id="workout-preview-title">Como a ficha será recebida</h2><p>Confira a ordem, os GIFs e os parâmetros antes de salvar ou enviar.</p></div><button type="button" aria-label="Fechar prévia" onClick={onClose}><X /></button></header><div className="workout-execution-preview-list">{selected.map((exercise, index) => { const detail = exerciseDetails[exercise.id]; const preview = exerciseGifSource(exercise); return <article className="student-execution-preview-card" key={exercise.id}><div className="student-execution-preview-index">{String(index + 1).padStart(2, "0")}</div><div className="student-execution-preview-media">{preview ? <img src={preview} alt={`Demonstração de ${exercise.name}`} /> : <span>Sem GIF</span>}</div><div className="student-execution-preview-copy"><small>{exercise.phase ?? "Treino principal"} · {exercise.muscleGroup}</small><h3>{exercise.name}</h3><p>{exercise.instructions || "Orientação objetiva será adicionada pelo professor."}</p><div className="student-preview-inline-metrics"><span><b>{detail?.sets ?? "3"}</b> séries</span><span><b>{detail?.reps ?? "10"}</b> repetições</span><span><b>{detail?.load ?? "0"}</b> kg</span><span><b>{detail?.rest ?? "60"}</b> s</span></div></div></article>; })}</div><button className="detail-save" type="button" onClick={onClose}>Fechar prévia</button></section></div>;
}
...
function TrainingModule({ onFeedback, initialStudentId = "" }: { onFeedback: (message: string) => void; initialStudentId?: string }) {
  ...
  function exportExerciseGifReport() {
    const rows = [
      ["Exercício", "Grupo muscular", "Região anatômica", "Status", "GIF masculino salvo", "GIF feminino salvo", "Caminho masculino", "Caminho feminino"],
      ...exercises.map((exercise) => {
        const maleSaved = exercise.gifMaleUrl || exercise.gifUrl || "";
        const femaleSaved = exercise.gifFemaleUrl || "";
        const maleCatalog = verifiedExerciseGifs[exerciseGifKey(exercise.name)]?.maleFile || "";
        const status = exerciseGifSource(exercise) ? "Com GIF disponível" : "Aguardando revisão manual";
        return [exercise.name, exercise.muscleGroup, exercise.anatomyRegion || "", status, maleSaved ? "Sim" : "Não", femaleSaved ? "Sim" : "Não", exercise.gifMalePath || exercise.gifPath || maleCatalog, exercise.gifFemalePath || ""];
      }),
    ];
    ...
    anchor.href = url;
    anchor.download = `relatorio-gifs-exercicios-${new Date().toISOString().slice(0, 10)}.csv`;
    anchor.click();
    URL.revokeObjectURL(url);
    onFeedback(`Relatório exportado com ${exercises.length} exercícios.`);
  }
  ...
  return <div className="workspace-content module-view">
    ...
    {workoutPreviewOpen && <WorkoutExecutionPreviewModal exercises={exercises} selectedExercises={selectedExercises} exerciseDetails={exerciseDetails} onClose={() => setWorkoutPreviewOpen(false)} />}
