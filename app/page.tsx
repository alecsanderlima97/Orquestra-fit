...
type WorkoutTemplateRecord = { id: string; name: string; level?: WorkoutLevel; audience?: WorkoutTemplateAudience; scheduleDay?: WorkoutTemplateDay; targetStudentId?: string | null; targetStudentName?: string | null; seedKey?: string; exerciseIds: string[]; exerciseDetails: WorkoutExerciseDetail[]; createdBy: string };
...
function TrainingModule({ onFeedback, initialStudentId = "" }: { onFeedback: (message: string) => void; initialStudentId?: string }) {
  ...
  async function seedGeneralPrograms() {
    ...
    levels.forEach((level, levelIndex) => dayFocus.forEach((day, dayIndex) => {
      ...
      createdTemplates.push({ id: `local-template-${seedKey}-${Date.now()}-${levelIndex}-${dayIndex}`, name: `${level} · ${day.day} · ${day.label}`, level, audience: "Geral", scheduleDay: day.day, targetStudentId: null, targetStudentName: null, seedKey, exerciseIds: selected.map((exercise) => exercise.id), exerciseDetails: selected.map(templateExerciseDetails), createdBy: access.userId });
    }));
