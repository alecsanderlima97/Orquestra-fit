      ...
      <div className="workout-list">
        {visibleWorkouts.length > 0 ? visibleWorkouts.map((workout, index) => (
          (() => {
            ...
            const stats = workoutCardStats(workout);
            const estimatedDuration = stats.estimatedSeconds > 0 ? `~${formatWorkoutDuration(stats.estimatedSeconds)}` : "—";
...
function workoutCardStats(workout: Pick<WorkoutRecord, "exerciseIds" | "exerciseDetails">) {
  const details = workout.exerciseDetails ?? [];
  let series = 0;
  let repetitions = 0;
  let estimatedSeconds = 0;
  details.forEach((exercise) => {
    const sets = Math.max(0, Number(exercise.sets) || 0);
    const reps = Math.max(0, Number(exercise.reps) || 0);
    const rest = Math.max(0, Number(exercise.rest) || 0);
    const metrics = exerciseMetricLabels(exercise);
    series += sets;
    if (metrics.mode === "cardio") {
      estimatedSeconds += sets * (reps * 60 + rest);
    } else if (metrics.mode === "timed") {
      estimatedSeconds += sets * (reps + rest);
    } else {
      repetitions += sets * reps;
      estimatedSeconds += sets * (Math.max(25, reps * 4) + rest);
    }
  });
  return { exerciseCount: workout.exerciseIds.length || details.length, series, repetitions, estimatedSeconds };
}
...
function PublishedWorkouts({ templates, workouts, onEditTemplate, onRemoveTemplate, onEditWorkout, onRemoveWorkout, onPreviewTemplate, onPreviewWorkout, onPrintTemplate, onSeedGeneralPrograms, targetStudentId, targetStudentName, onAssignTemplate }: { templates: WorkoutTemplateRecord[]; workouts: WorkoutRecord[]; onEditTemplate: (template: WorkoutTemplateRecord) => void; onRemoveTemplate: (template: WorkoutTemplateRecord) => void; onEditWorkout: (workout: WorkoutRecord) => void; onRemoveWorkout: (workout: WorkoutRecord) => void; onPreviewTemplate: (template: WorkoutTemplateRecord) => void; onPreviewWorkout: (workout: WorkoutRecord) => void; onPrintTemplate: (template: WorkoutTemplateRecord) => void; onSeedGeneralPrograms: () => void; targetStudentId?: string; targetStudentName?: string; onAssignTemplate?: (template: WorkoutTemplateRecord) => void }) {
  ...
  const renderTemplate = (template: WorkoutTemplateRecord) => {
    ...
    const stats = workoutCardStats(template);
    const estimatedDuration = stats.estimatedSeconds > 0 ? `~${formatWorkoutDuration(stats.estimatedSeconds)}` : "—";
