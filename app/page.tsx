...
function Agenda() {
  ...
  useEffect(() => {
    ...
    const unsubscribeClasses = onSnapshot(collection(db, "academies", access.academyId, "classes"), (snapshot) => {
      setClasses(snapshot.docs.map((item) => { const data = item.data() as Partial<ClassRecord>; return { id: item.id, name: data.name ?? "Aula", instructor: data.instructor ?? "Equipe", date: data.date ?? "", time: data.time ?? "", capacity: Number(data.capacity ?? 10), reservedCount: typeof data.reservedCount === "number" ? Math.max(0, data.reservedCount) : undefined, active: data.active !== false, status: data.status === "cancelled" ? "cancelled" as const : "scheduled" as const, visibility: data.visibility === "selected" ? "selected" as const : "open" as const, selectedStudentIds: Array.isArray(data.selectedStudentIds) ? data.selectedStudentIds : [] }; }).filter((item) => item.active));
    });
  ...
  function classStatus(item: ClassRecord, reserved: boolean) {
    if (item.status === "cancelled") return { key: "cancelled", label: "CANCELADA" };
  ...
  return (
    <div className="student-view">
      ...
      {selectedClasses.length === 0 ? <div className="empty-agenda"><CalendarDays /><h3>{visibleClasses.length === 0 ? "Nenhuma aula disponível" : "Nenhuma aula neste dia"}</h3><p>{visibleClasses.length === 0 ? "As próximas turmas da academia aparecerão aqui." : "Escolha outra data no calendário para consultar as aulas."}</p></div> : selectedClasses.map((item) => { const reserved = reservationIds.includes(item.id); const checkedIn = attendanceIds.includes(item.id); const status = classStatus(item, reserved); const isCompleted = status.key === "completed"; const isCancelled = status.key === "cancelled"; const isFull = status.key === "full"; const isWaitlist = status.key === "waitlist"; const spotsLeft = typeof item.reservedCount === "number" ? Math.max(0, item.capacity - item.reservedCount) : item.capacity; return <article className="class-card" key={item.id}><div className="class-time"><strong>{item.time}</strong><span>{isCancelled ? "Sem reservas" : isFull || isWaitlist ? "Sem vagas" : `${spotsLeft} ${spotsLeft === 1 ? "vaga" : "vagas"}`}</span></div><div><div className="class-card-heading"><small>{item.name.toUpperCase()}</small><span className={`agenda-class-status ${status.key}`}>{status.label}</span></div><h2>{item.name}</h2><p>{item.instructor} · {item.date}</p></div><div className="class-card-actions"><button disabled={isCompleted || isCancelled} onClick={() => { if (isCompleted || isCancelled) return; if (reserved) void cancel(item); else if (isWaitlist) void leaveWaitlist(item); else if (isFull) void joinWaitlist(item); else void reserve(item); }}>{isCancelled ? "Aula cancelada" : isCompleted ? "Aula encerrada" : reserved ? "Cancelar reserva" : isWaitlist ? "Sair da lista" : isFull ? "Entrar na lista de espera" : "Reservar"}</button>{reserved && !isCompleted && !isCancelled && <button className="secondary-action" disabled={checkedIn} onClick={() => void checkIn(item)}>{checkedIn ? "Presença registrada" : "Registrar presença"}</button>}</div></article>; })}
    </div>
  );
}
...
function ClassesModule({ onFeedback }: { onFeedback: (message: string) => void }) {
  const access = useAccess();
  const [classes, setClasses] = useState<ClassRecord[]>([]);
  const [waitlists, setWaitlists] = useState<ClassWaitlistRecord[]>([]);
  ...
  const [instructorId, setInstructorId] = useState(access.role === "teacher" ? access.userId : "");
  const [date, setDate] = useState("");
  ...
  useEffect(() => {
    ...
    return onSnapshot(collection(db, "academies", access.academyId, "classes"), (snapshot) => {
      setClasses(snapshot.docs.map((item) => {
        ...
        return { id: item.id, name: data.name ?? "Aula", instructor: data.instructor ?? "Equipe", instructorId: data.instructorId ?? null, date: data.date ?? "", time: data.time ?? "", capacity: Number(data.capacity ?? 10), reservedCount: typeof data.reservedCount === "number" ? Math.max(0, data.reservedCount) : undefined, active: data.active !== false, status: data.status === "cancelled" ? "cancelled" as const : "scheduled" as const, visibility: data.visibility === "selected" ? "selected" as const : "open" as const, selectedStudentIds: Array.isArray(data.selectedStudentIds) ? data.selectedStudentIds : [] };
  ...
  useEffect(() => {
    if (!db) {
      setWaitlists(readLocalCollection<ClassWaitlistRecord>(access.academyId, "classWaitlist"));
      return;
    }
    return onSnapshot(collection(db, "academies", access.academyId, "classWaitlist"), (snapshot) => {
      setWaitlists(snapshot.docs.map((item) => ({ id: item.id, ...(item.data() as Omit<ClassWaitlistRecord, "id">) })));
    }, (error) => console.error("Não foi possível carregar a fila de espera.", error));
  }, [access.academyId]);
  ...
  useEffect(() => {
    if (!db) {
      const localTeachers = readLocalCollection<RegisteredTeacher>(access.academyId, "teachers").filter((teacher) => teacher.active !== false);
      setTeachers(access.role === "teacher" ? localTeachers.filter((teacher) => teacher.id === access.userId || teacher.email === access.user.email) : localTeachers);
      return;
    }
    if (access.role === "teacher") {
      setTeachers([{ id: access.userId, name: accountName(access.user.displayName, access.user.email), email: access.user.email, active: true }]);
      return;
    }
    return onSnapshot(collection(db, "academies", access.academyId, "teachers"), (snapshot) => setTeachers(snapshot.docs.map((item) => {
      const data = item.data() as Partial<RegisteredTeacher>;
      return { id: item.id, name: data.name ?? "Professor sem nome", email: data.email ?? null, active: data.active !== false };
    }).filter((teacher) => teacher.active !== false)), (error) => console.error("Não foi possível carregar professores para a agenda.", error));
  }, [access.academyId, access.role, access.user.email, access.user.displayName, access.userId]);
  ...
  async function syncStaffReservations(classId: string, className: string, selectedIds: string[]) {
    ...
    if (!db) {
      ...
      if (visibility === "selected") {
        const nextClasses = readLocalCollection<ClassRecord>(access.academyId, "classes").map((item) => item.id === classId ? { ...item, reservedCount: next.filter((reservation) => reservation.classId === classId && reservation.status === "active").length } : item);
        writeLocalCollection(access.academyId, "classes", nextClasses);
    ...
    if (visibility === "selected") await updateDoc(doc(db, "academies", access.academyId, "classes", classId), { reservedCount: selected.size, updatedAt: serverTimestamp(), updatedBy: access.userId });
  }
  ...
  async function createClass(event: React.FormEvent<HTMLFormElement>) {
    ...
    setSaving(true);
    try {
      const data = { name: name.trim(), instructor: instructorName, instructorId: instructorId || null, date, time, capacity: Number(capacity) || 10, reservedCount: editingClassId ? (classes.find((item) => item.id === editingClassId)?.reservedCount ?? 0) : 0, status: editingClassId ? classes.find((item) => item.id === editingClassId)?.status ?? "scheduled" : "scheduled", visibility, selectedStudentIds: visibility === "selected" ? selectedStudentIds : [], updatedBy: access.userId, updatedAt: serverTimestamp() };
  ...
  function editClass(item: ClassRecord) {
    setEditingClassId(item.id); setName(item.name); setInstructorId(item.instructorId ?? teachers.find((teacher) => teacher.name === item.instructor)?.id ?? (access.role === "teacher" ? access.userId : "")); setDate(item.date); setTime(item.time); setCapacity(String(item.capacity));
  ...
  function clearForm() {
    setEditingClassId(null); setName(""); setInstructorId(access.role === "teacher" ? access.userId : ""); setDate(""); setTime(""); setCapacity("10"); setVisibility("open"); setSelectedStudentIds([]);
  }
  ...
  async function toggleCancellation(item: ClassRecord) {
    const nextStatus: ClassRecord["status"] = item.status === "cancelled" ? "scheduled" : "cancelled";
    if (!db) {
      const nextClasses = classes.map((current) => current.id === item.id ? { ...current, status: nextStatus } : current);
      setClasses(nextClasses);
      writeLocalCollection(access.academyId, "classes", nextClasses);
      onFeedback(nextStatus === "cancelled" ? "Aula marcada como cancelada no modo local." : "Aula reativada no modo local.");
      return;
    }
    try {
      await updateDoc(doc(db, "academies", access.academyId, "classes", item.id), { status: nextStatus, updatedAt: serverTimestamp(), updatedBy: access.userId });
      onFeedback(nextStatus === "cancelled" ? "Aula marcada como cancelada." : "Aula reativada.");
    } catch { onFeedback("Não foi possível alterar o status da aula."); }
  }
...
function PublishedWorkouts({ templates, workouts, onEditTemplate, onRemoveTemplate, onEditWorkout, onRemoveWorkout, onPreviewTemplate, onPreviewWorkout, onPrintTemplate, onSeedGeneralPrograms, targetStudentId, targetStudentName, onAssignTemplate }: { templates: WorkoutTemplateRecord[]; workouts: WorkoutRecord[]; onEditTemplate: (template: WorkoutTemplateRecord) => void; onRemoveTemplate: (template: WorkoutTemplateRecord) => void; onEditWorkout: (workout: WorkoutRecord) => void; onRemoveWorkout: (workout: WorkoutRecord) => void; onPreviewTemplate: (template: WorkoutTemplateRecord) => void; onPreviewWorkout: (workout: WorkoutRecord) => void; onPrintTemplate: (template: WorkoutTemplateRecord) => void; onSeedGeneralPrograms: () => void; targetStudentId?: string; targetStudentName?: string; onAssignTemplate?: (template: WorkoutTemplateRecord) => void }) {
  ...
  const renderTemplate = (template: WorkoutTemplateRecord) => {
    ...
    const templateIndex = orderedTemplates.findIndex((item) => item.id === template.id);
    const code = letterFromName(template.name) ?? String.fromCharCode(65 + Math.max(0, templateIndex));
    const alreadyAssigned = Boolean(targetStudentId && (assignedTemplateIds.has(template.id) || visibleWorkouts.some((workout) => workout.name === template.name && (workout.recommendedDay ?? "Flexível") === day)));
    return <div className="published-template-row" key={template.id}><div className="published-template-copy"><strong>Treino {code} <span className="published-template-slash">/</span> categoria {level}</strong><small className="published-template-name">{displayWorkoutCardName(template.name)}</small><div className="published-template-meta"><span className={`template-chip template-chip-level level-${machineCode(level)}`}><Trophy size={12} /> {level}</span><span className="template-chip template-chip-day"><CalendarDays size={12} /> {day}</span><span className="template-chip template-chip-focus"><Dumbbell size={12} /> {focus}</span><span className={audience === "Personalizado" ? "template-chip template-chip-audience personalized" : "template-chip template-chip-audience"}><Users size={12} /> {audience}</span><span className="template-chip template-chip-count"><ClipboardList size={12} /> {template.exerciseIds.length} exercícios</span></div>{target && <small className="published-template-target"><UserRoundCheck size={12} /> Direcionado para{target}</small>}</div><div className="published-item-actions">{targetStudentId && onAssignTemplate && audience === "Geral" && <button type="button" className={alreadyAssigned ? "published-add-template is-added" : "published-add-template"} disabled={alreadyAssigned} onClick={() => onAssignTemplate(template)}>{alreadyAssigned ? <><Check size={13} /> Adicionado</> : <><Plus size={13} /> Adicionar</>}</button>}<button type="button" onClick={() => onPreviewTemplate(template)}><Eye size={13} /> Visualizar</button><button type="button" onClick={() => onPrintTemplate(template)}><Printer size={13} /> Imprimir</button><button type="button" onClick={() => onEditTemplate(template)}>Editar</button><button type="button" onClick={() => onRemoveTemplate(template)}>Excluir</button></div></div>;
  };
  return <details className="workspace-panel published-workouts training-collapsible-card"><summary className="training-card-summary"><span><small>4 · MODELOS E PUBLICADOS</small><strong>{templates.length} modelos · {visibleWorkouts.length} fichas enviadas{targetStudentName ? ` para ${targetStudentName}` : ""}</strong><em>{targetStudentName ? `Escolha um modelo geral para adicionar a ${targetStudentName}.` : "Use filtros para encontrar rapidamente um programa geral ou personalizado."}</em></span><ChevronDown /></summary><div className="training-card-content">{empty ? <div className="directory-empty"><Dumbbell /><p>Crie um programa-base para reutilizá-lo com novos alunos.</p><button className="detail-secondary" type="button" onClick={onSeedGeneralPrograms}>Criar grade geral · 28 programas · 11 exercícios</button></div> : <><div className="published-list-toolbar"><label>Mostrar<select value={audienceFilter} onChange={(event) => setAudienceFilter(event.target.value as "Todos" | WorkoutTemplateAudience)}><option value="Todos">Todos os modelos</option><option value="Geral">Somente gerais</option><option value="Personalizado">Somente personalizados</option></select></label><label>Nível<select value={levelFilter} onChange={(event) => setLevelFilter(event.target.value as "Todos" | WorkoutLevel)}><option value="Todos">Todos os níveis</option><option>Fundação</option><option>Evolução</option><option>Performance</option><option>Elite</option></select></label><button className="detail-secondary" type="button" onClick={onSeedGeneralPrograms}>Criar/atualizar grade · 28 programas · 11 exercícios</button></div><div className="published-list">{orderedTemplates.length > 0 ? orderedTemplates.map(renderTemplate) : <div className="published-list-empty"><Dumbbell /><p>Nenhum modelo encontrado com esses filtros.</p></div>}</div>{visibleWorkouts.length > 0 && <details className="published-sent-list"><summary><span>Fichas já enviadas{targetStudentName ? ` para ${targetStudentName}` : ""}</span><strong>{visibleWorkouts.length}</strong></summary><div className="published-list">{orderedVisibleWorkouts.map((workout, index) => { const code = letterFromName(workout.name) ?? String.fromCharCode(65 + index); const level = workout.level ?? "Fundação"; const day = workout.recommendedDay ?? "Flexível"; const focus = workout.focusLabel ?? "Treino programado"; const audience = workout.audience ?? "Personalizado"; return <div className="published-template-row" key={workout.id}><div className="published-template-copy"><strong>Treino {code} <span className="published-template-slash">/</span> categoria {level}</strong><small className="published-template-name">{displayWorkoutCardName(workout.name)}</small><div className="published-template-meta"><span className={`template-chip template-chip-level level-${machineCode(level)}`}><Trophy size={12} /> {level}</span><span className="template-chip template-chip-day"><CalendarDays size={12} /> {day}</span><span className="template-chip template-chip-focus"><Dumbbell size={12} /> {focus}</span><span className="template-chip template-chip-audience personalized"><Users size={12} /> {audience}</span><span className="template-chip template-chip-count"><ClipboardList size={12} /> {workout.exerciseIds.length} exercícios</span></div><small>{workout.studentName}</small></div><div className="published-item-actions"><em>Enviado</em><button type="button" onClick={() => onPreviewWorkout(workout)}><Eye size={13} /> Visualizar</button><button type="button" onClick={() => printWorkoutSheet(workout)}><Printer size={13} /> Imprimir</button><button type="button" onClick={() => onEditWorkout(workout)}>Editar</button><button type="button" onClick={() => onRemoveWorkout(workout)}>Excluir</button></div></div>; })}</div></details>}</>}</div></details>;
}
...
function TrainingModule({ onFeedback, initialStudentId = "" }: { onFeedback: (message: string) => void; initialStudentId?: string }) {
  ...
  useEffect(() => {
    ...
    window.requestAnimationFrame(() => openTrainingCard("details.published-workouts"));
  }, [initialStudentId]);
  ...
  async function addTemplateToStudent(template: WorkoutTemplateRecord) {
    const targetId = initialStudentId || studentId;
    const student = students.find((item) => item.id === targetId);
    if (!student) {
      onFeedback("Selecione um aluno antes de adicionar o modelo.");
      return;
    }
    const studentUserId = student.userId ?? student.id;
    const alreadyAdded = workouts.some((workout) => workout.sourceTemplateId === template.id && (workout.studentRecordId === student.id || workout.studentId === studentUserId || workout.studentUserId === studentUserId));
    if (alreadyAdded) {
      onFeedback(`${template.name} já está adicionado para ${student.name}.`);
      return;
    }
  ...
  async function seedGeneralPrograms() {
    ...
    const dayFocus: Array<{ day: WorkoutTemplateDay; label: string; focus: string[]; bodyRegions: BodyRegion[] }> = [
      { day: "Segunda-feira", label: "Peito + tríceps", focus: ["Peito", "Tríceps"], bodyRegions: ["Tronco anterior", "Membros superiores"] },
      { day: "Terça-feira", label: "Costas + bíceps", focus: ["Costas", "Bíceps"], bodyRegions: ["Tronco posterior", "Membros superiores"] },
      { day: "Quarta-feira", label: "Quadríceps + glúteos", focus: ["Quadríceps", "Glúteos"], bodyRegions: ["Membros inferiores"] },
      { day: "Quinta-feira", label: "Ombros + core", focus: ["Ombros", "Core"], bodyRegions: ["Membros superiores", "Tronco anterior", "Região central"] },
      { day: "Sexta-feira", label: "Glúteos + posteriores", focus: ["Glúteos", "Posteriores"], bodyRegions: ["Membros inferiores"] },
      { day: "Sábado", label: "Panturrilhas + antebraço", focus: ["Panturrilhas", "Antebraço"], bodyRegions: ["Membros inferiores", "Membros superiores"] },
      { day: "Domingo", label: "Lombar + core", focus: ["Lombar", "Core"], bodyRegions: ["Tronco posterior", "Região central", "Membros inferiores"] },
    ];
    ...
    levels.forEach((level, levelIndex) => dayFocus.forEach((day, dayIndex) => {
      ...
      const selectedMain = day.focus.flatMap((term, focusIndex) => {
        const normalizedTerm = normalized(term);
        const exactMatches = mainExercises.filter((exercise) => normalized(exercise.muscleGroup) === normalizedTerm);
        const relatedMatches = mainExercises.filter((exercise) => {
          const text = `${exercise.name} ${exercise.muscleGroup} ${exercise.secondaryMuscles ?? ""}`;
          return normalized(text).includes(normalizedTerm) && !exactMatches.some((item) => item.id === exercise.id);
        });
        const candidates = [...exactMatches, ...relatedMatches];
        const rotation = (levelIndex * 2 + dayIndex + focusIndex) % Math.max(candidates.length, 1);
        const rotated = candidates.length ? [...candidates.slice(rotation), ...candidates.slice(0, rotation)] : [];
        return rotated.slice(0, 4);
      }).filter((exercise, index, list) => list.findIndex((item) => item.id === exercise.id) === index).slice(0, 8);
      const preparationCandidates = preparation.filter((exercise) => {
        const text = normalized(`${exercise.name} ${exercise.muscleGroup} ${exercise.secondaryMuscles ?? ""}`);
        return day.bodyRegions.includes(exercise.bodyRegion as BodyRegion) || day.focus.some((term) => text.includes(normalized(term)));
      });
      const preparationPool = preparationCandidates.length >= 2 ? preparationCandidates : preparation;
      const preparationRotation = (levelIndex + dayIndex) % Math.max(preparationPool.length, 1);
      const rotatedPreparation = preparationPool.length ? [...preparationPool.slice(preparationRotation), ...preparationPool.slice(0, preparationRotation)] : [];
      const selectedPreparation = rotatedPreparation.filter((exercise, index, list) => list.findIndex((item) => item.id === exercise.id) === index).slice(0, 2);
  ...
  return <div className="workspace-content module-view">
    ...
    <section className="training-layout training-layout-redesigned">
      <details className="workspace-panel training-form-panel workout-builder-panel training-collapsible-card">
        ...
        <label>Começar com programa-base<select defaultValue="" onChange={(event) => loadTemplate(event.target.value)}><option value="">Novo programa do zero</option>{templates.map((template) => <option key={template.id} value={template.id}>{template.audience ?? "Geral"} · {template.level ?? "Fundação"} · {displayWorkoutCardName(template.name)}</option>)}</select></label>
     ...
     <PublishedWorkouts templates={templates} workouts={workouts} targetStudentId={initialStudentId || undefined} targetStudentName={students.find((student) => student.id === (initialStudentId || studentId))?.name} onAssignTemplate={(template) => void addTemplateToStudent(template)} onEditTemplate={beginTemplateEdit} onRemoveTemplate={(template) => void removeTemplate(template)} onEditWorkout={beginWorkoutEdit} onRemoveWorkout={(workout) => void removeWorkout(workout)} onPreviewTemplate={previewTemplate} onPreviewWorkout={previewWorkout} onPrintTemplate={printWorkoutTemplate} onSeedGeneralPrograms={() => void seedGeneralPrograms()} />
        ...
        <aside className="workspace-panel student-detail-panel">
          {selectedStudent ? <>
            ...
            <label>Nome completo<input value={editName} onChange={(event) => setEditName(capitalizeName(event.target.value))} autoComplete="name" required /></label>
              ...
              <button className="detail-save" type="submit" disabled={saving}>{saving ? "Salvando..." : "Salvar alterações"}</button>
            </form> : <p className="student-profile-empty student-profile-readonly">A edição de dados pessoais e permissões é exclusiva da gestão.</p>}
